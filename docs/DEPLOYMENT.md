...
- 雅迪 VOC 案例页从占位页升级为完整作品集页面。
- 新增 `css/yadea.css`，用于雅迪案例页独立视觉和原型交互样式。
- 雅迪页面包含数据处理流程、标签体系与打标逻辑、样本验证、看板原型截面、分析路径、产品判断和项目结果。
- 标签体系模块采用作品集信息图结构，展示从原始社媒内容到结构化标签、长表拆分和原帖回溯的过程。
- 推送 `main` 后由 GitHub Actions 同步腾讯云 COS，并刷新 `https://www.zbxlab.cn/` CDN 缓存。
